/*
Language: JavaScript
Description: JavaScript (JS) language definition for highlight.js, extended with Jiki's repeat keyword.
...
const BUILT_INS = ([] as string[]).concat(
  BUILT_IN_GLOBALS,
  TYPES,
  ERROR_TYPES
);
...
export default function (hljs: HLJSApi): Language {
  ...
  const XML_TAG = {
    ...
    isTrulyOpeningTag: (match: RegExpMatchArray, response: { ignoreMatch: () => void }) => {
  ...
  SUBST.contains = SUBST_INTERNALS
    .concat({
      ...
      keywords: KEYWORDS$1,
      contains: [
        "self" as unknown as Mode
      ].concat(SUBST_INTERNALS)
    });
  const SUBST_AND_COMMENTS = ([] as Mode[]).concat(COMMENT, SUBST.contains!);
  const PARAMS_CONTAINS = SUBST_AND_COMMENTS.concat([
    {
      begin: /(\s*)\(/,
      end: /\)/,
      keywords: KEYWORDS$1,
      contains: ["self" as unknown as Mode].concat(SUBST_AND_COMMENTS)
    }
  ]);
  const PARAMS: Mode = {
    className: 'params',
